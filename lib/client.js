...
class WebMaxClient extends EventEmitter {
  constructor(options = {}) {
    ...
    // Получаем путь для сессий из опций
    const sessionPath = options.sessionPath || null;
	// Передаём путь в SessionManager
    this.session = new SessionManager(this.sessionName, sessionPath);
  ...
  async sendPong() {
    try {
      if (!this.ws || this.ws.readyState !== WebSocket.OPEN) {
            return; // соединение не активно — игнорируем
      }
